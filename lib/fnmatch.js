...
var translate = exports.translate = function(pattern) {
    ...
    return {
        "pattern": ["^", patternToRegex(pattern.substring(i)), "$"].join(""),
        "shouldMatch": !negated
    };
};

var fnmatch = exports.fnmatch = function(name, pattern) {
    var {pattern, shouldMatch} = translate(pattern);
    return (new RegExp(pattern, "i")).test(name) === shouldMatch;
};

exports.fnmatchcase = function(name, pattern) {
    var {pattern, shouldMatch} = translate(pattern);
    return (new RegExp(pattern)).test(name) === shouldMatch;
};
