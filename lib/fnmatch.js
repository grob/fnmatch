...
var cartesian = function cartesian(list) {
    ...
    var helper = function(arr, i) {
        for (let j=0, l=list[i].length; j<l; j+=1) {
            var a = arr.slice();
            a.push(list[i][j]);
            if (i === max) {
                r.push(a.join(""));
...
var patternToRegExp = exports.patternToRegExp = function(pattern, options) {
    var buf = [];
...
var make = exports.make = function(pattern, options) {
    ...
    return patternSets.map(function(pattern) {
        return pattern.split(PATH_SPLIT).map(function(segment) {
            ...
            return patternToRegExp(segment, options);
        });
    });
};
...
var matchPattern = function(segments, segmentIdx, patterns, patternIdx, options, partial) {
    ...
    while (segmentIdx < segmentsLen && patternIdx < patternsLen) {
        ...
        if (pattern === GLOBSTAR) {
            ...
            // console.log("Tested", pattern, "against", segment, "=>", result);
            if (result === false) {
                return false;
            }
        }
